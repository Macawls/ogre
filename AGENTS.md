...
The docs site is at `website/docs/` (Astro Starlight). The WASM playground binary is gitignored and must be built before deploying:

```bash
GOOS=js GOARCH=wasm go build -ldflags="-s -w" \
  -o website/docs/public/playground/ogre.wasm \
  ./cmd/playground/
```
