...
The docs site's OpenGraph card at `site/public/og.png` is generated by Ogre itself. To rebuild it after editing `site/scripts/og-template.html`:

```bash
cd site/scripts && ./gen-og.sh   # writes ../public/og.png
```

Commit the PNG output so the site build does not require the Ogre CLI in CI.
